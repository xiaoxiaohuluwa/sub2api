...
// The MemoFunc type is an adapter to allow the use of ordinary function as a Querier.
type MemoFunc func(context.Context, *ent.MemoQuery) (ent.Value, error)

// Query calls f(ctx, q).
func (f MemoFunc) Query(ctx context.Context, q ent.Query) (ent.Value, error) {
	if q, ok := q.(*ent.MemoQuery); ok {
		return f(ctx, q)
	}
	return nil, fmt.Errorf("unexpected query type %T. expect *ent.MemoQuery", q)
}

// The TraverseMemo type is an adapter to allow the use of ordinary function as Traverser.
type TraverseMemo func(context.Context, *ent.MemoQuery) error

// Intercept is a dummy implementation of Intercept that returns the next Querier in the pipeline.
func (f TraverseMemo) Intercept(next ent.Querier) ent.Querier {
	return next
}

// Traverse calls f(ctx, q).
func (f TraverseMemo) Traverse(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.MemoQuery); ok {
		return f(ctx, q)
	}
	return fmt.Errorf("unexpected query type %T. expect *ent.MemoQuery", q)
}
...
func NewQuery(q ent.Query) (Query, error) {
	...
	case *ent.MemoQuery:
		return &query[*ent.MemoQuery, predicate.Memo, memo.OrderOption]{typ: ent.TypeMemo, tq: q}, nil
